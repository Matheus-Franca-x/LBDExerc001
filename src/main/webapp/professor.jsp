...
<div class="bg-black custom">
	<div class="container bg-white scroll" id="nav-container">
		<h1 class="text-center">CRUD - Professor</h1>
		...
		<div class="form-container m-auto border border-primary rounded-4">
			<table class="table table-striped">
				<thead>
					<tr>
						<th class="col">Registro</th>
						<th class="col">Nome</th>
						<th class="col">Titulação</th>
					</tr>
				</thead>
				<tbody>
					<c:if test="${not empty professores}">
						<c:forEach var="c" items="${professores}">
							<tr>
								<th scope="row"><c:out value="${c.registro}"/></th>
								<td><c:out value="${c.nome}"/></td>
								<td><c:out value="${c.titulacao}"/></td>
							</tr>
						</c:forEach>
					</c:if>
				</tbody>
